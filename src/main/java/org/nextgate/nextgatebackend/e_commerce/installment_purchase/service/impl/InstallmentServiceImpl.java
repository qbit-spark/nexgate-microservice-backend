...
public class InstallmentServiceImpl implements InstallmentService {
    ...
    public InstallmentAgreementEntity createInstallmentAgreement(
        ...
        boolean requiresImmediateOrder =
                savedAgreement.getFulfillmentTiming() == FulfillmentTiming.IMMEDIATE;

        if (requiresImmediateOrder) {

            log.info("Fulfillment: IMMEDIATE");
            log.info("  → Order will be created asynchronously");
            log.info("  → Product will ship once order is processed");
        ...
        // ========================================
        try {
            InstallmentAgreementCreatedEvent event =
                    new InstallmentAgreementCreatedEvent(
                            this,
                            savedAgreement.getAgreementId(),
                            checkoutSession.getSessionId(),
                            savedAgreement,
                            requiresImmediateOrder  // true = create order now, false = defer
                    );

            eventPublisher.publishEvent(event);

            log.info("✓ InstallmentAgreementCreatedEvent published");
            if (requiresImmediateOrder) {
                log.info("  → Order creation will proceed asynchronously");
            }

        } catch (Exception e) {
            log.error("Failed to publish InstallmentAgreementCreatedEvent", e);
            // Don't throw - agreement created successfully
        }
    ...
    public InstallmentPaymentEntity processInstallmentPayment(UUID paymentId)
        ...
        /*
         * Processes an installment payment (scheduled or manual)
         *
         * HANDLES:
         * - Full payments (entire scheduledAmount)
         * - Completing partially paid installments (from flexible payments)
         *
         * FLOW:
         * 1. Fetch and validate payment
         * 2. Calculate remaining amount to charge
         * 3. Check wallet balance
         * 4. Execute payment via ledger
         * 5. Update payment and agreement records
         * 6. Handle agreement completion if final payment
         */
        ...
        log.info("  Remaining: {} TZS", payment.getRemainingAmount());
        log.info("  Due Date: {}", payment.getDueDate());
        log.info("  Current Status: {}", payment.getPaymentStatus());
        log.info("  Is Partially Paid: {}", payment.isPartiallyPaid());

        // Check if already completed
        if (payment.getPaymentStatus() == PaymentStatus.COMPLETED) {
            log.info("✓ Payment already completed - returning existing record");
            return payment;
        }

        // ========================================
        // STEP 2: CALCULATE AMOUNT TO CHARGE
        // ========================================
        BigDecimal requiredAmount = payment.getRemainingAmount();

        if (requiredAmount.compareTo(BigDecimal.ZERO) <= 0) {
            log.info("✓ Payment has no remaining balance - marking as complete");
        ...
        if (payment.isPartiallyPaid()) {
            log.info("║  TYPE: Completing Partial Payment                          ║");
            log.info("║  Previously Paid: {} TZS                                   ",
                    String.format("%,.2f", payment.getPaidAmount()));
        } else {
            log.info("║  TYPE: Full Payment                                        ║");
        }
        log.info("╚════════════════════════════════════════════════════════════╝");
        ...
        log.info("  Required: {} TZS", String.format("%,.2f", requiredAmount));
        log.info("  Available: {} TZS", String.format("%,.2f", walletBalance));
        ...
        log.info("  Source (Wallet): {} (Balance: {} TZS)",
                walletLedger.getAccountNumber(),
                String.format("%,.2f", walletLedger.getCurrentBalance()));
        log.info("  Destination (Platform): {} (Balance: {} TZS)",
                platformRevenue.getAccountNumber(),
                String.format("%,.2f", platformRevenue.getCurrentBalance()));

        // Create description
        String description = String.format(
                "Installment payment #%d of %d for agreement %s (%s)%s",
                payment.getPaymentNumber(),
                agreement.getNumberOfPayments(),
                agreement.getAgreementNumber(),
                agreement.getProductName(),
                payment.isPartiallyPaid() ? " - Completing partial payment" : ""
        );

        log.info("Transaction Description: {}", description);
        log.info("Creating ledger entry for {} TZS...",
                String.format("%,.2f", requiredAmount));

        // Execute atomic money transfer via double-entry bookkeeping
        LedgerEntryEntity ledgerEntry = ledgerService.createEntry(
                walletLedger,                           // DEBIT (money leaves wallet)
                platformRevenue,                        // CREDIT (money enters platform)
                requiredAmount,                         // AMOUNT (remaining amount only)
        ...
        log.info("  Amount Transferred: {} TZS", String.format("%,.2f", ledgerEntry.getAmount()));
        log.info("  Entry Type: {}", ledgerEntry.getEntryType());
        log.info("  New Wallet Balance: {} TZS",
                String.format("%,.2f", walletLedger.getCurrentBalance()));
        ...
        if (payment.isPartiallyPaid()) {
            // Completing a partially paid installment
            log.info("Completing partially paid installment:");
            log.info("  Previously Paid: {} TZS",
                    String.format("%,.2f", payment.getPaidAmount()));
            log.info("  This Payment: {} TZS",
                    String.format("%,.2f", requiredAmount));

            BigDecimal newTotalPaid = payment.getPaidAmount().add(requiredAmount);
            ...
            log.info("  Total Paid: {} TZS",
                    String.format("%,.2f", newTotalPaid));

        } else {
            // Recording full payment
            log.info("Recording full payment completion");
        ...
        log.info("  Payment Method: WALLET");
        ...
        // IMPORTANT: Record the FULL scheduled amount in agreement
        // Even if we only charged the remaining amount this time
        // Because agreement tracks "payments completed", not "amount charged per transaction"
        ...
        log.info("  Amount Paid: {} → {} TZS",
                String.format("%,.2f", previousAmountPaid),
                String.format("%,.2f", agreement.getAmountPaid()));
        log.info("  Amount Remaining: {} → {} TZS",
                String.format("%,.2f", previousAmountRemaining),
        ...
        if (agreement.getNextPaymentDate() != null) {
            log.info("  Next Payment Due: {} (Amount: {} TZS)",
                    ...
                    String.format("%,.2f", agreement.getNextPaymentAmount()));
        } else {
        ...
        if (agreement.getOrderId() != null) {
            ...
            log.info("  Amount Paid: {} TZS", String.format("%,.2f", agreement.getAmountPaid()));
            log.info("  Total Amount: {} TZS", String.format("%,.2f", agreement.getTotalAmount()));
        ...
        if (payment.isPartiallyPaid()) {
            log.info("  Message: 'Your partially paid installment #{} is now complete!'",
                    payment.getPaymentNumber());
        } else {
            log.info("  Message: 'Payment #{} of {} processed successfully!'",
                    payment.getPaymentNumber(),
                    agreement.getNumberOfPayments());
        }
        ...
        // ========================================
        long processingDuration = java.time.Duration.between(
                payment.getAttemptedAt(), now).toMillis();
        ...
        log.info("  Amount Charged: {} TZS", String.format("%,.2f", requiredAmount));
        log.info("  Processing Duration: {} ms", processingDuration);
        log.info("  Retry Count: {}", payment.getRetryCount());
        log.info("  Completion Type: {}",
                payment.isPartiallyPaid() ? "Partial Completion" : "Full Payment");
...
// ========================================
   ...
   // ========================================
    ...
    public void handleAgreementCompletion(InstallmentAgreementEntity agreement) {
